...
router.get('/', function(req, res) {
    ...
    .then(company => Promise
      ...
      .map(user => user
        .promise_vacation()
        .then(vacation_obj => Promise.resolve({
          user_row : user,
          vacation_available : vacation_obj.available,
          vacation_used : vacation_obj.used,
        })),
        {
          concurrency : 10
        }
...
function promise_user_list_data_for_rendering(args) {
  ...
  const usersInfoForRendering = users_info.map(ui => ({
    ...
    vacation_available: ui.vacation_available,
    vacation_used: ui.vacation_used,
