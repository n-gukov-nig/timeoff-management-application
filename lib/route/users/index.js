...
router.get('/add/', function (req, res) {
  req.user.get_company_for_add_user().then(function (company) {
    res.render('user_add', {
      company: company,
...
router.get('/edit/:user_id/', function (req, res) {
  ...
  Promise.try(() => ensure_user_id_is_integer({ req: req, user_id: user_id }))
    .then(() => req.user.get_company_for_user_details({ user_id }))
    .then(function (company) {
      const employee = company.users[0];

      return employee.promise_schedule_I_obey().then(function () {
...
router.get('/edit/:user_id/absences/', function (req, res) {
  let user_id = validator.trim(req.params['user_id']),
    user_allowance;
...
router.get('/edit/:user_id/schedule/', function (req, res) {
  ...
  Promise.try(function () {
    ...
    .then(function (company) {
      ...
      return employee.promise_schedule_I_obey().then(function (schedule) {
        res.render('user_details', {
          employee: employee,
          schedule: schedule,
...
router.get('/edit/:user_id/calendar/', async (req, res) => {
  ...
  const currentYear = moment.utc(moment(), 'YYYY');
  ...
  try {
    ...
    userAllowance = await employee.promise_allowance({ year: currentYear });
  } catch (error) {
    console.error(
...
var ensure_user_was_not_useed_elsewhere_while_being_inactive = function (args) {
  ...
  if (
    ...
    (!new_user_attributes.end_date ||
      ...
      (new_user_attributes.end_date &&
        moment.utc(new_user_attributes.end_date).startOf('day').toDate() >=
          req.user.company.get_today().startOf('day').toDate()))
  ) {
    return model.User.find_by_email(new_user_attributes.email).then(function (
      user
    ) {
...
router.post('/edit/:user_id/', function (req, res) {
  ...
  Promise.try(function () {
    ...
    .catch(function (error) {
      console.error(
        'An error occurred when trying to save chnages to user account by user ' +
          req.user.id +
          ' : ' +
          error
      );
  ...
  req.user
    ...
    .then(function (company) {
      company.users.forEach(function (user) {
        ...
        user.my_leaves.forEach(function (leave) {
          leave.user = user;
        });
      });
...
function promise_user_list_data_for_rendering(args) {
  ...
  const usersInfoForRendering = users_info.map((ui) => ({
    ...
    number_of_days_taken_from_allowance:
      ui.user_row.calculate_number_of_days_taken_from_allowance(),
