...
function get_instance_methods(sequelize) {
  return {
    ...
    promise_vacation: function (args) {
      args = args || {};
      args.user = this;
      return Vacation.promise_vacation(args);
    },
    ...
    reload_with_leave_details: function (args) {
      ...
      return Promise.join(
        self
          ...
          .promise_my_active_leaves_ever(args)
          .then((leaves) =>
    ...
    get_reset_password_token: function () {
      ...
      return new Buffer(
        self.email + ' ' + self.Model.hashify_password(self.password)
      ).toString('base64');
    },
    ...
    get_active_user_filter: function () {
      return {
        $or: [
          ...
          {
            end_date: {
              $gte: moment.utc().startOf('day').format('YYYY-MM-DD'),
            },
          },
        ],
      };
