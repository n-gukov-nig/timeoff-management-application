...
const schema_user = Joi.object().required(),
  ...
  schema_promise_allowance = Joi.object()
    .required()
    .keys({
      year: schema_year,
      user: schema_user,
...
class UserAllowance {
  ...
  get total_number_of_days_in_allowance() {
    return Math.floor(this.days_in_allowance);

    // Old logic for total_number_of_days_in_allowance
  ...
  get days_in_allowance() {
    const self = this;

    return (self.nominal_allowance * (this.total_work_days / 365)).toFixed(2);
  }

  get total_work_days() {
    const self = this;
    const now = self._now.clone();
    const date = now > moment.utc(self.user.end_date) ? moment.utc(self.user.end_date) : now
    return Math.ceil(
      moment.duration(date).asDays() -
        moment.duration(moment.utc(self.user.start_date)).asDays()
    );
  }
  ...
  get number_of_days_available_in_allowance() {
    ...
    return (
      this.days_in_allowance -
      self.number_of_days_taken_from_allowance +
      (self.is_accrued_allowance ? self.accrued_adjustment : 0)
    ).toFixed(2);
  }

  get days_available_in_allowance() {
    return Math.trunc(this.number_of_days_available_in_allowance)
  }
  ...
  static promise_allowance(args) {
    ...
    flow = flow.then(() => {
      ...
      // console.log("flow args", JSON.stringify(args, null, 2))
